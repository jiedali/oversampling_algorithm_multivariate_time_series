...
"""
this script compares performance from data generated at end of epoch 0,1,2,3,4, in one-shot
"""
##########
...
#
for epoch in range(1,1+n_epochs,1):

	print("=============Running epoch %d Comparison==============" % epoch)
	...
	for i in range(0, 20):
		# returns synthetic samples from both the first epoch and the last epoch
		clusters, likelihoods, sample_likelihoods, history, new_samples_all_clusters, new_samples_c0_epoch0, \
		new_samples_c1_epoch0, new_samples_c0_last_epoch, new_samples_c1_last_epoch = train_new_gmm(
			pos_low_d_transposed, neg_low_d_transposed, n_clusters, n_epochs, 0.01, num_em_samples, eigen_signal)
		...
		last_f1_score, last_precision, last_recall, last_gmean = workflow1.workflow_70_inos(
			num_ADASYN=num_adasyn_samples, train_p=train_p, train_n=train_n,
			new_samples_all_clusters=new_samples_last_epoch, remove_tomeklinks=False, model_name=model_name)
		second_epoch_f1.append(last_f1_score)
		second_epoch_precision.append(last_precision)
		second_epoch_recall.append(last_recall)
		second_epoch_gmean.append(last_gmean)
		...
		first_f1_score, first_precision, first_recall, first_gmean = workflow1.workflow_70_inos(
			num_ADASYN=num_adasyn_samples, train_p=train_p, train_n=train_n,
		...
		first_epoch_gmean.append(first_gmean)
	# do classification with samples at epoch 0

	results_df['first_epoch_f1'] = first_epoch_f1
	results_df['second_epoch_f1'] = second_epoch_f1
	results_df['first_epoch_recall'] = first_epoch_recall
	results_df['second_epoch_recall'] = second_epoch_recall
	results_df['first_epoch_precision'] = first_epoch_precision
	results_df['second_epoch_precision'] = second_epoch_precision
	results_df['first_epoch_gmean'] = first_epoch_gmean
	results_df['second_epoch_gmean'] = second_epoch_gmean

	results_df.to_csv('./results/em_comparison/em_comprison_Libras_epoch_'+str(epoch)+'.csv')